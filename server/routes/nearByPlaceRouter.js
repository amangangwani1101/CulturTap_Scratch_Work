...
// Define a function to get nearby places based on the genre
async function getNearbyPlaces(req, res, genreName) {
  try {
    console.log(`User searching for nearby ${genreName}`);
    ...
    // Query MongoDB to find nearby places within the 20 km range
    const genre = await GenreModel.findOne({ name: genreName });

    if (!genre) {
      res.status(404).json({ error: `${genreName} genre not found` });
      return;
    }

    const nearbyPlaces = await SingleStoryModel.find({
      _id: { $in: genre.stories }, // Filter by the references in the specified genre
    ...
    // Group nearby places by a 100m range
    const groupedPlaces = {};
    ...
    const placesWithAvgRating = Object.values(groupedPlaces).map((place) => {
      ...
      return place.stories.map((story) => ({
        ...
        transportationPricing: story.transportationPricing,
        selectedOption: story.selectedOption,
        businessCategory: story.businessCategory,
        genre: story.genre,
        likes: story.likes,
        views: story.views,
        userID: story.userID,
    ...
    // Send the sorted nearby places with complete story data as a JSON response
...
// Define routes for different genres
router.get('/nearby-places/:genre', async (req, res) => {
  const genreName = req.params.genre; // Assuming the genre name in the URL is lowercase
  await getNearbyPlaces(req, res, genreName);
