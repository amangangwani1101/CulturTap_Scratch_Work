...
// Define a route to get nearby riverside places
router.get('/api/nearby-riverside-places', async (req, res) => {
  console.log('user riverside dekhne aya hai');
  try {
    ...
    // Query MongoDB to find nearby riverside places within the 20 km range
    const riversideGenre = await GenreModel.findOne({ name: 'Riverside' });

    if (!riversideGenre) {
      res.status(404).json({ error: 'Riverside genre not found' });
      return;
    }

    const nearbyRiversidePlaces = await SingleStoryModel.find({
      _id: { $in: riversideGenre.stories }, // Filter by the references in the "Riverside" genre
    ...
    // Group nearby riverside places by a 100m range
    const groupedPlaces = {};

    nearbyRiversidePlaces.forEach((place) => {
    ...
    // Send the sorted nearby riverside places with complete story data as a JSON response
