...
// Define a route to get nearby island places
router.get('/api/nearby-island-places', async (req, res) => {
  console.log('user islands dekhne aya hai');
  try {
    ...
    // Query MongoDB to find nearby island places within the 20 km range
    const islandCategory = await GenreModel.findOne({ name: 'Island' });

    if (!islandCategory) {
      res.status(404).json({ error: 'Island category not found' });
      return;
    }

    const nearbyIslandPlaces = await SingleStoryModel.find({
      category: islandCategory.name, // Filter by the "Island" category
    ...
    // Group nearby island places by a 100m range
    const groupedPlaces = {};

    nearbyIslandPlaces.forEach((place) => {
    ...
    // Send the sorted nearby island places with complete story data as a JSON response
