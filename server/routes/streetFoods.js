...
router.get('/api/nearby-street-foods', async (req, res) => {
  ...
  try {
    ...
    // Query MongoDB to find nearby street food places within the 20 km range
    const streetFoodGenre = await GenreModel.findOne({ name: 'Street Foods' });

    if (!streetFoodGenre) {
      res.status(404).json({ error: 'Street Foods genre not found' });
      return;
    }

    const nearbyStreetFoodPlaces = await SingleStoryModel.find({
      _id: { $in: streetFoodGenre.stories }, // Filter by the references in the "Street Foods" genre
    ...
    // Group nearby street food places by a 100m range
    const groupedPlaces = {};

    nearbyStreetFoodPlaces.forEach((place) => {
    ...
    // Send the sorted nearby street food places with complete story data as a JSON response
