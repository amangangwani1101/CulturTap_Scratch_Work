...
// Define a route to get nearby national parks
router.get('/api/nearby-national-parks', async (req, res) => {
  console.log('user national park dekhne aya hai');
  try {
    ...
    // Query MongoDB to find nearby national parks within the 20 km range
    const nationalParksGenre = await GenreModel.findOne({ name: 'National Parks' });

    if (!nationalParksGenre) {
      res.status(404).json({ error: 'National Parks genre not found' });
      return;
    }

    const nearbyNationalParks = await SingleStoryModel.find({
      _id: { $in: nationalParksGenre.stories }, // Filter by the references in the "National Parks" genre
    ...
    // Group nearby national parks by a 100m range
    const groupedPlaces = {};

    nearbyNationalParks.forEach((place) => {
    ...
    // Send the sorted nearby national parks with complete story data as a JSON response
