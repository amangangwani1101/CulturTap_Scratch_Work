...
// Define a route to get nearby aquatic ecosystem places
router.get('/api/nearby-aquatic-ecosystem-places', async (req, res) => {
  console.log('user aquatic ecosystem dekhne aya hai');
  try {
    ...
    // Query MongoDB to find nearby aquatic ecosystem places within the 20 km range
    const aquaticEcosystemGenre = await GenreModel.findOne({ name: 'Aquatic Ecosystem' });

    if (!aquaticEcosystemGenre) {
      res.status(404).json({ error: 'Aquatic Ecosystem genre not found' });
      return;
    }

    const nearbyAquaticEcosystemPlaces = await SingleStoryModel.find({
      _id: { $in: aquaticEcosystemGenre.stories }, // Filter by the references in the "Aquatic Ecosystem" genre
    ...
    // Group nearby aquatic ecosystem places by a 100m range
    const groupedPlaces = {};

    nearbyAquaticEcosystemPlaces.forEach((place) => {
    ...
    // Send the sorted nearby aquatic ecosystem places with complete story data as a JSON response
