...
// Define a route to get nearby forests
router.get('/api/nearby-forests', async (req, res) => {
  console.log('user forests dekhne aya hai');
  try {
    ...
    // Query MongoDB to find nearby forests within the 20 km range
    const forestsGenre = await GenreModel.findOne({ name: 'Forests' });

    if (!forestsGenre) {
      res.status(404).json({ error: 'Forests genre not found' });
      return;
    }

    const nearbyForests = await SingleStoryModel.find({
      _id: { $in: forestsGenre.stories }, // Filter by the references in the "Forests" genre
    ...
    // Group nearby forests by a 100m range
    const groupedPlaces = {};

    nearbyForests.forEach((place) => {
    ...
    // Send the sorted nearby forests with complete story data as a JSON response
